...
/**
 * Created by Zhao Qing on 2018/5/30.
 * hdfs.SplitAndSelectThroughputCount
 */
public class SplitAndSelectThroughputCount {
    public static void main(String[] args) throws Exception{
        ...
        Job job = Job.getInstance(conf, "SplitAndSelectThroughputCount");//创建一个job
        job.setJarByClass(SplitAndSelectThroughputCount.class);//指定主类
    ...
    public static class TimeStampMapper
        ...
        public void map(Object key, Text value, Context context
            ...
            while (itr.hasMoreTokens()) {
                ...
                word.set(strs[0]);//设置键值为时间戳（秒级）
                count.set(Long.parseLong(strs[1]));
                context.write(word, count);//Context.write(输出KEY,输出VALUE)-->生成一个输出的键值对
            }
        }
    }
